...
var scroller = (function($){
	...
	var queue = [],
		...
		getCategoryStr = function(catStr) {
			//var cats = catStr.split(', ');
			return catStr.split(', ').join('&nbsp;&nbsp;|&nbsp;&nbsp;');
		},
		
		loadMovies = function() {
			var page = nextPage;
			var remaining = movieCount - (perPage * page);
			var count = remaining <= (perPage-1) && remaining >= 1 ? remaining : perPage;
			
			if(remaining < perPage || movieCount == (perPage * (page+1))) {
				complete = true;	
			}
			...
			for(var i = 0; i < count; i++) {
				...
				(function(index){$.getJSON( "http://www.omdbapi.com/?i=" + movies[(page * perPage) + i], 
		...
		updateDOM = function() {
			...
			var movie = queue.shift(),
				output = "";
				
			while(typeof movie !== 'undefined') {
				
				// render the template to output
				
				output += '<div class="movie">' +
				'<div class="movie-img"><img src="' + movie.Poster + '" /></img></div>' +
				'<h1>' + movie.Title + '</h1><span>( ' + movie.Year + ' )</span>' +
				'<span class="nobr">' +
				'<span class="rating ' + getRatingClass(movie.Rated) + '"></span>' +
				movie.Runtime + '&nbsp;&nbsp;&nbsp;&nbsp;' + getCategoryStr(movie.Genre) + '-&nbsp;&nbsp;&nbsp' + movie.released +
				'</span>' +
				'<span class="nobr">' +
				'<span>Ratings: ' + movie.imdbRating + '/10 from ' + movie.imdbVotes + ' users</span>' +
				'</span>' +
				'<p>' + movie.Plot + '</p>' +
				'</div>';
